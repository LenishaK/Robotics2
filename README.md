...
## 1.	Project overview 
This project focuses on the design and development of an automated robot capable of transporting bins from one location to another. The system uses a VEX EXP robot combined with programmed control logic to simulate  real-world industrial task. 
...
The final demonstrated system uses VEX EXP robot, its drive motors, arm motor, claw motor and inertial sensor to complete the task automatically. A Raspberry Pi and camera module were also explored during development extention for visual sensing. 
...
The final demonstrated solution runs on the VEX EXP robot using VEXcode Python. Separate Pi camera code was also developed during the project as an extention.
...
This algorithm uses programmed robot movement and inertial-senor-based heading control to perform the task automatically in a controlled environment.
...
5. Place the items at pickup points A, B and C.
6. Run the program
...
A Raspberry Pi camera subsystem was explored during development as a possible extension, but it was not included in the final live demonstration. 
# 8. Limitations 
